...
tests/
  server/                    API, configuration, and database integration tests
  frontend/                  Resume parsing and file validation tests
scripts/                     Development launcher and admin maintenance
docs/ai.md                   Gemini configuration and troubleshooting
```

`node_modules/` contains installed dependencies, `dist/` contains the production
build, and `tmp/` holds local diagnostics, screenshots, logs, and archived generated
files. These are working artifacts, not application source. `vite.config.ts` is the
...
- `npm run build` checks types and builds the frontend into `dist/`.
...
The template API tests are now in `tests/server/templates.test.js`.
...
node --test tests/server/templates.integration.test.js
