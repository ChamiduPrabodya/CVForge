...
function fixture(t) {
  ...
  writeFileSync(join(directory, ".env"), "GEMINI_API_KEY=base-test-key\nPORT=4000\n");
  return directory;
}

test("local settings take precedence over base settings, but retain a real deployment key", t => {
  const directory = fixture(t);
  writeFileSync(join(directory, ".env.local"), "GEMINI_API_KEY=local-test-key\n");
  assert.deepEqual(loadEnvironment({ directory, env: {} }), { GEMINI_API_KEY: "local-test-key", PORT: "4000" });
  const env = { GEMINI_API_KEY: "deployment-test-key" };
  loadEnvironment({ directory, env });
  assert.equal(env.GEMINI_API_KEY, "deployment-test-key");
});

test("empty inherited keys no longer hide a configured key", t => {
  const directory = fixture(t);
  for (const value of ["", "   "]) {
    const env = { GEMINI_API_KEY: value };
    loadEnvironment({ directory, env });
    assert.equal(env.GEMINI_API_KEY, "base-test-key");
  }
});
...
test("can load a settings file created after the first configuration read", t => {
  ...
  writeFileSync(join(directory, ".env.local"), "GEMINI_API_KEY=added-later-test-key\n");
  loadEnvironment({ directory, env });
  assert.equal(env.GEMINI_API_KEY, "added-later-test-key");
});
